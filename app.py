...
def initialiser_base():
    ...
    """)
    cursor.execute(
        "INSERT OR IGNORE INTO utilisateurs (role, code) VALUES ('admin', '1234')"
    )
    cursor.execute(
        "INSERT OR IGNORE INTO utilisateurs (role, code) VALUES ('reception', '0000')"
    )
...
class ApplicationBMProd:
    ...
    def verifier_acces(self, niveau_requis):
        ...
        if niveau_requis == "admin":
            ...
            cursor.execute(
                "SELECT code FROM utilisateurs WHERE role IN ('admin', 'reception') AND code=?",
                (code_saisi,),
            )
    ...
    def verifier_reliquat_telephone(self, event=None):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT SUM(somme_restante) FROM artistes WHERE telephone = ? AND statut != 'PAYER'",
                (numero,),
            )
    ...
    def rafraichir_avances(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, nom_artiste, numero, date_versement, somme_versee, moyen_versement, date_prevue FROM avances ORDER BY id DESC"
        )
    ...
    def rafraichir_table_artistes(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT nom_artiste, date_enregistrement, titre_chanson, heure, telephone, ville, type_prod, style_musical, tarifs, somme_versee, somme_restante, statut, num_facture FROM artistes ORDER BY id DESC"
        )
        for row in cursor.fetchall():
    ...
    def ajouter_conso_elec(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO conso_electricite (date_conso, index_kwh, montant, notes) VALUES (?, ?, ?, ?)",
            (
                self.e_el_dt.get(),
                self.e_el_idx.get(),
                mnt,
    ...
    def ajouter_depense(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO depenses (date_depense, categorie, description, montant) VALUES (?, ?, ?, ?)",
            (
                self.e_dp_dt.get(),
                self.e_dp_cat.get(),
                self.e_dp_dsc.get(),
    ...
    def creer_ecran_donnees_quotidien(self):
        ...
        cursor.execute(
            "SELECT date_conso, index_kwh, montant, notes FROM conso_electricite ORDER BY id DESC"
        )
        for r in cursor.fetchall():
            t1.insert("", "end", values=r)

        cursor.execute(
            "SELECT date_depense, categorie, description, montant FROM depenses ORDER BY id DESC"
        )
        for r in cursor.fetchall():
    ...
    def rafraichir_nos_depenses(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, achat, nombre, prix_unitaire, total FROM nos_depenses ORDER BY id DESC"
        )
        for r in cursor.fetchall():
    ...
    def filtrer_base_artistes(self, event=None):
        ...
        query = "SELECT nom_artiste, date_enregistrement, titre_chanson, heure, telephone, ville, type_prod, style_musical, tarifs, somme_versee, somme_restante, statut, num_facture FROM artistes WHERE 1=1"
        params = []

        if txt:
            query += " AND (nom_artiste LIKE ? OR telephone LIKE ? OR CAST(num_facture AS TEXT) LIKE ?)"
    ...
    def charger_donnees_facture(self, event=None):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT nom_artiste, date_enregistrement, titre_chanson, heure, telephone, ville, type_prod, style_musical, tarifs, somme_versee, somme_restante, statut, num_facture FROM artistes WHERE CAST(num_facture AS TEXT) = ?",
            (num_str,),
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def creer_bloc_recu(self, d, tampon_titre):
        ...
        tk.Label(
            f_leg,
            text="BIG S MEDIA PRODUCTION | NIF: 1495556/P | RCCM: NE-01-2025-A-03906//22/09/2025 | LICENCE: Entrepreneur Culturel/C 0207",
    ...
    def imprimer_facture(self):
        ...
        try:
            ...
            if not d:
                messagebox.showerror("Erreur", f"Aucune facture trouvée pour le N° {num_str}")
                return
            ...
            logo_html = ""
            logo_paths = ["logo.png", "logo.jpg", "LOGO 1.jpg"]
            logo_file = None
            ...
            if logo_file:
                with open(logo_file, "rb") as img_f:
                    ...
                    logo_html = f'<img src="data:{mime};base64,{b64_data}" style="height: 45px; vertical-align: middle; margin-right: 10px;">'

            html_content = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Facture N° {d[12]}</title>
    <style>
        @page {{ size: A4; margin: 8mm; }}
        body {{ font-family: Arial, sans-serif; font-size: 12px; color: #000; margin: 0; padding: 0; }}
        .recu-box {{ border: 2px solid #000; padding: 12px; margin-bottom: 8px; background: #fff; }}
        .header {{ display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #c0392b; padding-bottom: 6px; margin-bottom: 10px; }}
        .title-area {{ display: flex; align-items: center; }}
        .title {{ font-size: 15px; font-weight: bold; color: #c0392b; }}
        .badge {{ font-size: 13px; font-weight: bold; background: #f1c40f; padding: 4px 10px; border: 1px solid #000; }}
        .content-table {{ width: 100%; border-collapse: collapse; margin-bottom: 8px; }}
        .content-table td {{ padding: 5px; vertical-align: top; }}
        .label-col {{ font-weight: bold; width: 22%; }}
        .val-col {{ border: 1px solid #000; padding: 5px 8px; background: #f9f9f9; width: 28%; }}
        .footer {{ text-align: center; font-size: 9px; margin-top: 8px; border-top: 1px dashed #7f8c8d; padding-top: 4px; }}
        .cut-line {{ text-align: center; font-style: italic; color: #7f8c8d; margin: 10px 0; font-size: 11px; }}
    </style>
</head>
<body>
    ...
    <div class="recu-box">
        <div class="header">
            <div class="title-area">
                {logo_html}
                <span class="title">BM PROD - REÇU DE PAIEMENT (EXEMPLAIRE CLIENT)</span>
            </div>
            <span class="badge">N° FACTURE : {d[12]}</span>
        </div>
        <table class="content-table">
            <tr>
                <td class="label-col">Téléphone :</td>
                <td class="val-col">{d[4]}</td>
                <td class="label-col" style="padding-left: 15px;">Tarif Total :</td>
                <td class="val-col">{t_val}</td>
            </tr>
            <tr>
                <td class="label-col">Nom d'Artiste :</td>
                <td class="val-col">{d[0]}</td>
                <td class="label-col" style="padding-left: 15px;">Somme Versée :</td>
                <td class="val-col">{v_val}</td>
            </tr>
            <tr>
                <td class="label-col">Date session :</td>
                <td class="val-col">{d[1]}</td>
                <td class="label-col" style="padding-left: 15px;">Statut :</td>
                <td class="val-col"><b>{d[11]}</b></td>
            </tr>
            <tr>
                <td class="label-col">Titre chanson :</td>
                <td class="val-col">{d[2]}</td>
                <td class="label-col" style="padding-left: 15px;">Restant à payer :</td>
                <td class="val-col">{r_val}</td>
            </tr>
            <tr>
                <td class="label-col">Heure d'entrée :</td>
                <td class="val-col">{d[3]}</td>
                <td class="label-col" style="padding-left: 15px;">Style musical :</td>
                <td class="val-col">{d[7]}</td>
            </tr>
            <tr>
                <td class="label-col">Ville :</td>
                <td class="val-col">{d[5]}</td>
                <td class="label-col" style="padding-left: 15px;">Genre musical :</td>
                <td class="val-col">{d[6]}</td>
            </tr>
        </table>
        <div class="footer">
            <b>BIG S MEDIA PRODUCTION</b> | NIF: 1495556/P | RCCM: NE-01-2025-A-03906//22/09/2025 | LICENCE: Entrepreneur Culturel/C 0207<br>
            Tél: +227 96 55 81 93 | Email: sanistomoussa@gmail.com
        </div>
    </div>

    <div class="cut-line">- - - - - - - - - - - - - - - - - - - - - LIGNE DE COUPE - - - - - - - - - - - - - - - - - - - - -</div>
    ...
    <div class="recu-box">
        <div class="header">
            <div class="title-area">
                {logo_html}
                <span class="title">BM PROD - REÇU DE PAIEMENT (EXEMPLAIRE STUDIO)</span>
            </div>
            <span class="badge">N° FACTURE : {d[12]}</span>
        </div>
        <table class="content-table">
...
"""
            ...
            messagebox.showinfo(
                "Succès",
                f"Le reçu avec logo pour la facture N° {num_str} a été généré !",
            )
        except Exception as e:
